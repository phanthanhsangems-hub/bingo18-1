...
def _query_combo_watch(db) -> tuple:
    ...
    try:
        ...
        cur.execute(
            ...
            "ORDER BY draw_number DESC LIMIT 2"
        )
        rows = cur.fetchall()
        conn.close()
        if len(rows) < 2:
        ...
        def _combo(raw):
            ...
            return ''.join(str(x) for x in sorted(int(v) for v in nums))

        return int(rows[0][0]), _combo(rows[0][1]), _combo(rows[1][1])
    except Exception as e:
        logger.debug("466 watch query error: %s", e)
        return None, None, None
